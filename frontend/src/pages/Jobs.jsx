...
const Jobs = () => {
  ...
  // Debounce text inputs (search, location) by 400ms
  // Selects and date fire immediately via handleInstantFilterChange
  useEffect(() => {
  ...
  // Fetch jobs — depends on debouncedFilters, NOT filters
  const fetchJobs = useCallback(async () => {
    try {
      ...
      if (debouncedFilters.type)
        queryParams.append("jobType", debouncedFilters.type);
      if (debouncedFilters.salary) {
        const [min, max] = debouncedFilters.salary.split("-");
      ...
      if (debouncedFilters.datePosted)
        queryParams.append("datePosted", debouncedFilters.datePosted);
  ...
  }, [fetchSavedJobIds, fetchUserProfile, fetchRecommendations]);
