...
- Collaborate with back-end developers and web designers`,
		requirements: [
			...
			"Unit testing experience with Jest",
		],
		skills_required: [
			"React",
			"JavaScript",
...
async function seed(pool) {
	...
	for (const job of jobs) {
		...
		if (companyResult.rows.length === 0) {
			console.log(
				`   Skipping job "${job.title}" - company "${job.company_name}" not found`,
			);
			continue;
		}
	...
	console.log(
		`   Created ${created} job postings, skipped ${skipped} existing`,
	);
}
