...
function runPythonRecommendation(action, inputData) {
  return new Promise((resolve, reject) => {
    const pythonScript = path.join(
      __dirname,
      "../utils/ai/recommendation_engine.py",
    );
...
const getAIRecommendations = async (req, res) => {
  try {
    ...
    const jobsResult = await pool.query(
      `SELECT
        j.id, j.title, j.description, c.name as company_name, j.location, j.job_type,
        j.experience_level, j.salary_min, j.salary_max, j.currency,
...
async function getBasicRecommendations(userId) {
  ...
  if (
    ...
    const recentResult = await pool.query(
      `SELECT j.id, j.title, j.description, c.name as company_name, j.location, j.job_type,
              j.experience_level, j.salary_min, j.salary_max, j.currency, j.skills_required
       FROM job_postings j
       LEFT JOIN companies c ON j.company_id = c.id
       WHERE j.is_active = true
       ORDER BY j.created_at DESC
  ...
  const scoredJobs = jobsResult.rows
    .map((job) => {
      ...
      if (userSkills.length > 0) {
        const jobSkills = (job.skills_required || []).map((s) =>
          s.toLowerCase(),
        );
        ...
        const skillScore =
          jobSkills.length > 0 ? (matches.length / jobSkills.length) * 50 : 0;
        score += skillScore;
      }
      ...
      if (preferredTypes.length > 0 && job.job_type) {
        ...
        const typeMatch = preferredTypes.some(
          (t) =>
            t.toLowerCase().includes(jobTypeLower) ||
            jobTypeLower.includes(t.toLowerCase()),
        );
      ...
      if (preferredLocations.length > 0 && job.location) {
        ...
        const locationMatch = preferredLocations.some(
          (l) =>
            ...
            l.toLowerCase().includes(locationLower),
        );
